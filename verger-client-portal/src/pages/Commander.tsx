...
export default function Commander() {
  ...
  useEffect(() => {
    if (!pickupPoint || !basketType || baskets.length === 0) {
      setSelectedBasket(null);
      return;
    }

    if (basketType === "personnalise") {
      // Sélection fictive pour personnalisé
      setSelectedBasket({
        id: 0,
        name: "Panier Personnalisé",
        price: 0,
  ...
  }, [pickupPoint, basketType, baskets, weeklyProducts]);

  const [loadingBaskets, setLoadingBaskets] = useState(true);

  useEffect(() => {
    if (basketType !== "personnalise") {
      // Si on quitte le panier personnalisé, réinitialiser les sélections
      setCustomProductsSelected({});
      setCustomProductQuantities({});
      setWeeklyProducts([]);
    }
  }, [basketType]);
  ...
  useEffect(() => {
    ...
    async function loadWeeklyProducts() {
      ...
      try {
        ...
        // Produits à exclure par nom exact
        const excludedNames = [
          "ACTIVATION FIDELITE",
          "Down Payment (POS)",
          "Booking Fees",
        ...
        const filteredProducts: BasketProduct[] = products
          .filter(
            (p) => !excludedNames.includes(p.name) && !p.name.startsWith("m"),
          )
          .map((p) => ({
            id: p.id,
            name: p.name,
        ...
        setWeeklyProducts(filteredProducts);
        setSelectedBasket({
  ...
  const totalPrice =
    basketType === "personnalise"
      ...
      : basketType === "decouverte"
        ? 12.99 * (isSubscription ? 4 : 1)
        ...
        basketType === "personnalise"
          ? selectedBasket?.products
              ...
              .map((p) => ({
                ...
                price: p.price || 0, // ✅ toujours récupérer le prix correct
              }))
          : undefined;

      // 🟢 CAS SPÉCIAL : Panier Découverte = ABONNEMENT UNIQUEMENT
      if (basketType === "decouverte") {
        ...
        navigate("/confirmation", {
          state: {
            orderNumber: `SUB-${subResult.subscriptionId}`,
            customer: formData,
            pickupPoint: pickupPoints.find((p) => p.id === pickupPoint),
            basket: { name: "Panier Découverte (Abonnement)" },
            isSubscription: true,
            totalPrice,
          },
        });

        return; // ⛔️ TRÈS IMPORTANT : on stoppe ici
      }

      // 🔵 TOUS LES AUTRES PANIERS = COMMANDE NORMALE
              ...
              <Card className="animate-fade-in">
                ...
                <CardContent>
                  <RadioGroup value={basketType} onValueChange={setBasketType}>
                    {/* 1️⃣ Paniers pré-fabriqués */}
                    <h3 className="font-semibold text-lg mb-2 mt-4">
                      Paniers Pré-fabriqués de la semaine
                    </h3>
                    <div className="grid md:grid-cols-3 gap-6">
                      {["fruits", "legumes", "mixte"].map((id) => {
                        const basket = basketsUI.find((b) => b.id === id);
                        ...
                        return (
                          ...
                          >
                            ...
                            />
                            <div className="p-4 bg-card flex flex-col gap-1">
                    ...
                    {/* 2️⃣ Panier personnalisé */}
                    <h3 className="font-semibold text-lg mb-2 mt-6">
                      Panier Personnalisé
                    </h3>
                    <div className="grid md:grid-cols-3 gap-6">
                      {basketsUI
                        .filter((b) => b.id === "personnalise")
                        .map((basket) => (
                          <label
                            key={basket.id}
                            className={`relative overflow-hidden rounded-2xl border-2 cursor-pointer transition-all flex flex-col items-stretch ${
                    ...
                    {/* 3️⃣ Panier Découverte / abonnement */}
                    <h3 className="font-semibold text-lg mb-2 mt-6">
                      Panier Abonnement hebdomadaire
                    </h3>
                    <div className="grid md:grid-cols-3 gap-6">
                      {basketsUI
                        .filter((b) => b.id === "decouverte")
                        .map((basket) => (
                          <label
                            key={basket.id}
                            className={`relative overflow-hidden rounded-2xl border-2 cursor-pointer transition-all flex flex-col items-stretch ${
                          ...
                          >
                            ...
                            <div className="p-4 bg-card flex flex-col gap-1">
                              ...
                              <span className="text-sm text-muted-foreground text-center">
                                Paiement et livraison chaque semaine
                              </span>
                            </div>
                            {basketType === basket.id && (
                              <div className="absolute top-2 right-2 w-6 h-6 bg-primary rounded-full flex items-center justify-center">
            ...
            {step === 3 && (
              <Card className="animate-fade-in">
                <CardHeader>
                  ...
                  <CardDescription>
                    {basketType === "decouverte"
                      ? "Panier Découverte : abonnement hebdomadaire avec livraison chaque semaine"
                      : basketType === "personnalise"
                        ? "Sélectionnez vos produits personnalisés pour composer votre panier"
                ...
                <CardContent>
                  ...
                  {basketType === "decouverte" && (
                    <div className="text-center py-16">
                      <ShoppingBasket className="w-14 h-14 mx-auto text-primary mb-6" />
                      <h3 className="text-2xl font-semibold mb-4">
                        Panier Découverte 🌿
                      </h3>
                      <p className="text-muted-foreground max-w-lg mx-auto">
                        Contenu surprise chaque semaine selon la récolte. <br />
                        Livraison et paiement automatique chaque semaine tant
                        que l’abonnement est actif.
                      </p>
                    </div>
                  )}

                  {/* 🟡 Paniers pré-fabriqués */}
                  {basketType !== "decouverte" &&
                    basketType !== "personnalise" &&
                    selectedBasket?.products && (
